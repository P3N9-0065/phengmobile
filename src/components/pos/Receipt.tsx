...
export function Receipt({ data }: { data: ReceiptData }) {
  return (
    <div id="pos-receipt" className="font-mono text-[12px] text-black bg-white p-3 w-[80mm] mx-auto">
      <div className="text-center border-b border-dashed border-black pb-2 mb-2">
        <h2 className="font-bold text-[14px]">ເພັງ ໂມບາຍ</h2>
        <p className="text-[10px]">Pheng Mobile</p>
        <p className="text-[10px] mt-1">ໃບເສັດຮັບເງິນ</p>
      </div>
      ...
      <div className="border-t border-dashed border-black pt-2 mt-2 space-y-1">
        ...
        <div className="flex justify-between font-bold text-[13px] border-t border-black pt-1">
        ...
        {data.currency_paid !== "LAK" && (
          <>
            ...
            <div className="flex justify-between text-[10px] text-gray-600"><span>ອັດຕາ:</span><span>1 {data.currency_paid} = {formatLAK(data.exchange_rate)}</span></div>
          </>
        )}
        {data.currency_paid === "LAK" && (
      ...
      <div className="text-center mt-3 pt-2 border-t border-dashed border-black text-[10px]">
        <p>ຂອບໃຈທີ່ໃຊ້ບໍລິການ</p>
        <p className="mt-1">ເພັງ ໂມບາຍ Pheng Mobile</p>
      </div>
    </div>
  );
}
...
<style>
  ...
  body { margin: 0; font-family: 'Noto Sans Lao', monospace; }
  #pos-receipt { width: 80mm; padding: 8px; }
